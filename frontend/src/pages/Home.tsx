...
let parseData = fetch(
  ...
  }).then((json) => JSONLD.parse(json).then((parseResult) => {
...
export default function Home() {
  ...
  return (
    <div className={style.Home+' unreset'}>
      ...
      <ol>
        <li>
          Take an example FOAF profile in JSONLD, like <a href="https://github.com/idafensp/idafensp.github.io/blob/da79fb6c87978719daa94a50f97c555a4e993f91/foaf.jsonld" target={"_blank"}>this one</a> on github (first one I found)
          {/*<div className={style.codeBlock}>*/}
          {/*  {json ? <CodeView of={Code.fromValue(json)} language="json" /> : '...'}*/}
          {/*</div>*/}
        </li>
        <li>
          Since this document describes a <a href="http://xmlns.com/foaf/0.1/#term_Person" target={"_blank"}>foaf:Person</a>, fetch the JSONLD and retrieve the person described in it as an an instance of the <a href="https://www.lincd.org/browse/modules/lincd-foaf/shapes/Person" target={"_blank"}>Person</a> shape (from the lincd-foaf package)
          <CodeView className={style.codeBlock} lineNumbers={false} of={Code.fromValue(fetchPersonCode)} language="typescript" />
        </li>
        <li>
           Visualise this person with any of the matching components with 1 line of code:
          <CodeView className={style.codeBlock} lineNumbers={false} of={Code.fromValue(visualizePersonCode)} language="typescript" />
        </li>
        <li>
           The result:
      ...
      <h4>Full code:</h4>
      <CodeView className={style.codeBlock+' '+style.fullCodeBlock} lineNumbers={false} of={Code.fromValue(fullCode)} language="typescript" />
    </div>
  );
}
